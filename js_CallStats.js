...
function create_match_display(start,end)
{
    ...
    for (i = start; i < end; i++) //gameId.length = end
    {
    	if (win[i] == "Win")
    	{
    ...
    tableString += "<button class='plus_icon' id='plus_icon'>";
    tableString += "<br><br><br><br><br><br><br><br><br><br>";
...
function more_match_display()
{
	...
	/*var	elementToRemove = document.getElementById("gamelist");
	if (elementToRemove != null)
	{
		document.getElementById("pre_gamelist").removeChild(document.getElementById("gamelist"));
	}*/
...
/*function update_date(x)
{
	...
	var month = String(d.getMonth()+1); // ATTENTION UN +1 Degeu ici
	if (0 <= month && month < 10)
	{
		month = "0" + month;
	}
...
function last_traitement()
{
    ...
    if (plus_called == 1)
    {
    	create_match_display(0,NbMatch);
    }
    else
    {  	
...
function display_graph()
{
    ...
    var vsmin_reversed = vsmin.reverse();
    var kda_reversed = kda.reverse();
    var kp_reversed = kp.reverse();
    var csmin_reversed = csmin.reverse();
    var deaths_reversed = deaths.reverse();
	...
	else
	{
	   	...
	   	if (ChampionValue != "Ø Champion")
    	{
			ajaxGet(create_url_get_matches_AWS(AccountId,convert_region(Region),"90","0"),traitementMatches);
	   		//ajaxGet(create_url_get_matches_by_date_AWS(AccountId,convert_region(Region),Today_Date-86400000,Today_Date),traitementMatches);
    	}
	   	else
	   	{
...
function first_call()
{
    ...
    //NbMatch = document.getElementById("NbMatch_Bis").value;
    ...
    //document.getElementById("NbMatch").value =  NbMatch;
